...
| Field | Value |
|---|---|
| **Active milestone** | **M10 — extensions.** v1 (M0–M9) complete; **M10.2b (pilot caching) gate passed 2026-07-16** |
| **Status** | ✅ 922 tests green (814 fast + 108 slow) · ruff + mypy --strict clean · **the pilots are content-addressed**: `prepare_model` 23.08 s → **1.17 s** warm (19.6×), `T₁` key and `s_J` bit-identical cold vs warm vs no-cache, 19 MB stored. The `--cache-dir` pilot DAG runs end-to-end from the CLI with `T₁` certified (3.87e-11, 26× inside contract) before the scale pilot steps in its frame |
| **Next action** | **M10.2c — overlap `prepare_model` with sampling across models. The plan already settles this and the code disagrees; the tracker's own recorded remedy was the weaker one.** §1.2 says: "Process models so their per-model geometry (sequential, cached) can overlap the **sampling** of earlier models, but feed one global worker pool." The pool *is* global ✓ — but `run_batch` does `for spec in specs: _run_one_model(...)`, and `_run_one_model` blocks on every future before returning, so model B's 23 s prepare never overlaps model A's sampling. **This tracker recorded "two-phase pool dispatch" (parallelise a pilot's 4 chains) instead: ≤4× on the serial term, 4 of 14 workers busy — while §1.2's overlap hides the term *entirely* for a batch of N>1, which is the use case this package exists for.** Measure before building (the M10.1/M10.2 rule, now 3-for-3). ⚠️ **Second, measured, and the same family: `cache.ArtifactCache` has exactly two layers in use — `L3` and the new `pilot`.** Nothing stores or loads L0/L1/L2, and the `file_lookup` key this file describes ("skip re-parsing across runs") **does not exist**. Warm `prepare_model` is **1.17 s and essentially all cobra parsing** — i.e. the whole remaining serial term is a stage the docs claim is cached. That is now the DAG's dominant serial cost and M9's "the code never implemented its own documentation", a third time. Then: a **metadata digest in `cache.ArtifactCache`** (it hashes every array and *trusts the meta* — deferred by agreement, an all-layer property not an L3 one); β→performance calibration (spec §22.3 — now cheap); the **span certificate's RNG-marginal gate** (M9, below); the β>0 kernel's 18% of unused work (M9); parallel tempering; slice kernel; mode-feature discovery. |
| **Blockers** | none |
