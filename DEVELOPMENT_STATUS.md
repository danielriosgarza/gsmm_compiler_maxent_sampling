...
| Field | Value |
|---|---|
| **Active milestone** | 🔴 **M11 — v1 on the real batch.** **M11.0 (the L3 lookup key names its solver) gate passed 2026-07-17.** *The previous line here read "v1 (M0–M9) complete"; it was measured on one model and it was wrong — see below.* |
...
| **Next action** | 🔴 **M11.1 — `blocked_reactions`' third state + one bounded cold escalation (BUILD_PLAN §1.6.11).** It is the largest failure (22 + 9 = **31 of the 36 broken strains**) and its premise is **measured**: a paired warm-vs-cold census over all 40 strains moves 22 AMBIGUOUS→OK and 9 kUnknown→OK with **0 regressions on 7 controls**. `kUnknown` yields *no witness*, so the crash and the misclassification are the **same case** — unresolved → one cold retry → if still unresolved, **report the resolution rather than choose a class**. The residue is both *Hafnia alvei* strains (`n_blocked` identical warm and cold, `U ≈ 2e-9` vs a 1e-9 bar): genuinely unresolved, which is what the third state is for. **Gate: refusals become a function of the model, not the seed.** Requires `/collab` (M4, math-critical). Then **M11.2** span → `resolution ≤ span_tol` (12/12 seeds pass, incl. both current failures, 37× margin); **M11.3** reachability's dual-witness path (CERTIFIED, 27× inside contract); **M11.4** re-run the census; **M11.5** release (LICENSE, README/docs, a manifest that reads the real `strains.tsv`). ⚠️ **The biggest unknown is downstream and unmeasured**: the census tests `blocked_reactions` only — rounding, the pilot DAG and the sampler have **never run on an aerobe**. |
...
#### 🔴 M11.1 — `blocked_reactions`: the third state + one bounded cold escalation  *(NEXT — 31 of the 36 broken strains)*
- [ ] 🤝 `/collab` **before** the build (M4, math-critical): the escalation policy and the three-state contract
- [ ] the third state — certified-blocked / certified-moving / **unresolved**; `U > blocked_tol` licenses *"not certified blocked by this dual"*, **not** "moving"
- [ ] `kUnknown` yields *no witness* → **unresolved**, so the 9 crashes and the 22 misclassifications are the **same case**; caught caller-side, **without loosening `HighsLinearProgram.solve()`** (`critical_l1_penalty` needs `LPNotOptimalError` to detect an unbounded `J*`)
- [ ] one bounded cold retry on an unresolved *decisive* solve; if still unresolved, **report the resolution rather than choose a class**. Fixed-period `clearSolver` is **rejected**: no justified `K`, and reaction 380 is tight *after* loose predecessors — path sensitivity, not age
- [ ] gate: the 40-strain census reruns; **refusals become a function of the model, not the seed**; the 2 *Hafnia* strains get an honest verdict

#### ⬜ M11.2 — span gates on `resolution ≤ span_tol`  *(2 strains, and the ~12% seed lottery)*
