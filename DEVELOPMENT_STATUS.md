...
| Field | Value |
|---|---|
| **Active milestone** | **M9 — Performance & GSMM hardening** |
| **Status** | ⬜ NOT STARTED (M0–M8 gates passed; 812 tests green) |
| **Next action** | Benchmark suite (parse→CSC→passModel→first LP→warm-start LPs→sparse LP→geometry→rounding→β=0 sps→β>0 sps→breakpoint dist→output); worker-count sweep {1,2,4,7,14} on the Jetson by ESS/wall-sec across the batch; allocation + breakpoint-sort profiling; validate the `reduced` storage mode end-to-end; assert the performance invariants (no per-step HiGHS, no scipy, no Python loop in chord, no element-wise highspy extraction, no full reconstruction every step). |
| **Blockers** | none |
| **Last updated** | 2026-07-16 |

> **M9 is not math-critical** — no `/collab` gate is required. Use `/collab` only for a genuine design
...
.venv/bin/python -m pytest -q | tail -3                # expect 812 passed
