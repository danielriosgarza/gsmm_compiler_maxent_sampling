...
| Field | Value |
|---|---|
| **Active milestone** | 🔴 **M11 — v1 on the real batch.** **M11.0–M11.3 gates passed** (build-geometry OK **4 → 40 of 40**); **M11.4 end-to-end census done** on a 4-strain sentinel spread (d=46/51/71/145) — *the pipeline is correct on aerobes; the cost is efficiency.* **M11.5 (release + production-hardening) is next.** *The line that read "v1 (M0–M9) complete" was measured on one model and was wrong — see below.* |
| **Status** | ✅ **979 tests green** (+8). ruff + mypy --strict clean. **build-geometry now completes on 40 of 40 real curated strains** (was 4 when M11 opened): M11.0 named the solver in the L3 cache key; M11.1 gave `blocked_reactions` a three-state classifier + fully-cold escalation (4→20); M11.2 changed the span gate to `resolution ≤ span_tol` + a build-wide solve session that eliminated every `kUnknown@flux_only` (20→30); **M11.3 gave the reachable-mass-balance certificate a caller-specific dual-witness path** so it reads a `kUnknown` solve's sound duals instead of refusing on the one output it never reads (30→40). Root cause of the whole M11 family: a persistent warm-started HiGHS instance degrades over hundreds of sequential re-solves — its primal path is history-sensitive while its duals stay sound-but-loose. Full reasoning: **BUILD_PLAN §1.6.11**. ⚠️ **The "40" honestly**: the **6 reachability fixes are durable/machine-independent** (the fix removes the failure mode); the **4 previously-deferred** strains (2 *Hafnia*, pumilus, Liquorilactobacillus) pass on this machine as **basis/RNG-marginal** (§1.6.10) and can flip on another basis. ⚠️ **On branch `m11-real-batch` (not yet merged to `main`).** ✅ **M11.4: the full pipeline (rounding → pilots → sampler) now HAS run on aerobes** (d=51/71/145) — samples valid, invariance + monotonicity hold; the cost is efficiency (mixing/`s_J` precision degrade ~linearly with d). |
| **Next action** | 🔴 **M11.5 — release + production-hardening.** The M11.4 census (**[benchmarks/M11_4_CENSUS.md](benchmarks/M11_4_CENSUS.md)**) found **no correctness defect** on aerobes — feasibility, T₀/T₁ parameterization invariance, and mean-J monotonicity all hold at d up to 145 — so what remains is hardening, not gates: **(a)** a **dimension-scaled sampling schedule** — **build spec ready at [.collab/specs/m115-dimension-scaled-schedule.md](.collab/specs/m115-dimension-scaled-schedule.md)** (mixing efficiency and `s_J` precision both degrade ~linearly with d; d=46 base under-powered on aerobes; doubling confirms ESS ∝ sweeps). ⚠️ **Ordering is in the spec and must not be reordered: MEASURE τ across a wider strain set first (a rule fitted to the 4 census sentinels is a guess), then `/collab` think on the fixed-vs-target-ESS-vs-doubling fork, then build.** The trap the spec flags: the *resolved* schedule must flow into the sample `content_key` (as `energy_scale_value` does for `s_J`), or two runs of one config collide in the cache. Not a math gate (the schedule changes *how many* draws, not the target law). **(b)** release (LICENSE, README/docs, a manifest that reads the real `strains.tsv`); **(c)** a full 40-strain production census + geometry-pilot covariance study + `s_J` under an independent pilot seed. Deferred, unchanged: the geometry residues (2 *Hafnia* blocked floor, pumilus √k span floor, Liquorilactobacillus basis-dependent span) and the **M11.3 residual charge** (dual-independent objective-normalization term in `_reachable_extreme`, ~1e-25). |
