...
| Field | Value |
|---|---|
| **Active milestone** | **M10 — extensions.** v1 (M0–M9) complete; **M10.2e (geometry determinism) gate passed 2026-07-17** |
| **Status** | ✅ **954 tests green (+16), and green *regardless of the ambient thread count* — which is the milestone.** ruff + mypy --strict clean. **The suite and the CLI are both green from a clean state for the first time since M10.2a**: `maxent sample` completes from an empty cache under default threading (4/4 units, `T₁` certified 3.87e-11). **The L3 artifact is now a function of its key** — `numerics.deterministic_blas` forces + scopes BLAS to 1 thread around the three constructors that need it, and the L3 key carries `DETERMINISM_POLICY_VERSION` so caches holding an ambient-thread basis miss. It **costs nothing — it pays**: `build_geometry` 1.170 s pinned vs 1.488 s at 14 threads (**21% faster**). Also: `prepare_model` 22.9 s → **0.645 s** warm (35×), a warm run **never imports cobra**. |
| **Next action** | **M10.2c — overlap `prepare_model` with sampling across models. §1.2 mandates *this* one** ("process models so their per-model geometry can overlap the **sampling** of earlier models"): the pool is global ✓, but `run_batch` is `for spec in specs: _run_one_model(...)` and blocks on every future, so **no overlap exists**. ⚠️ *Check that claim against §1.2's own words before building on it* — M10.2e is the cautionary tale: the last entry that said "§1.2 mandates it and the code drifted" was **wrong**, and a rule that has just paid off is the one you will over-apply next. The tracker's own recorded "two-phase pool dispatch" (parallelise a pilot's 4 chains) is the **weaker** remedy — ≤4×, 4 of 14 workers busy. Then: a **metadata digest in `cache.ArtifactCache`** (it hashes every array and *trusts the meta* — deferred by agreement, an all-layer property; note M10.2e's `numerical_identity` re-derives its own `recipe_key` on load rather than trusting it, which is the pattern); β→performance calibration (spec §22.3 — now cheap); the **span certificate's RNG-marginal gate** (M9, below); the β>0 kernel's 18% of unused work (M9); parallel tempering; slice kernel; mode-feature discovery. |
| **Blockers** | ✅ **None.** M10.2e cleared the standing one: `maxent sample` completes from a clean cache under default threading, and this file's Verify commands no longer need a threads-pinned workaround. |
| **Carried, not chased** | ⚠️ **`certify_reachable_mass_balance` is fragile, and M10.2e *hid* this rather than fixing it — by agreement.** One of the two (both valid) bases yields a `T₁` whose certificate LP returns **kUnknown** — a solver failure on a demonstrably valid input, and the failing `T₁` is *better* conditioned than the passing one (5352 vs 5969), so it cannot be blamed on unlucky geometry. Pinning the threads makes that basis unreachable **by default**; any other model, seed or machine can still land on it. This is M9's own finding resurfacing in M9's own replacement gate: *a bar a valid input clears only 2 times in 3 is not a tolerance, it is a coin flip.* It needs `certify`'s **LP formulation** looked at, not the thread count. Reproduction: build the basis with `build_geometry.__wrapped__` under ambient threads (bypassing the M10.2e policy — this is what `__wrapped__` is for), then `certify_reachable_mass_balance(T₁, reduced)`. |
