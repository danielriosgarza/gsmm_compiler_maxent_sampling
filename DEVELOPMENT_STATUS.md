...
| Field | Value |
|---|---|
| **Active milestone** | **v1 COMPLETE — M0–M9 gates all passed** |
| **Status** | ✅ 847 tests green · ruff + mypy --strict clean · [benchmarks/M9_REPORT.md](benchmarks/M9_REPORT.md) produced |
| **Next action** | **M10** (deferred extensions), whose first two items are now *prerequisites* rather than nice-to-haves: (1) **pilot-based `s_J`** — without it the β axis does not mean what a reader assumes (M6); (2) **pilot rerounding** — measured 2.5× ESS (M5). Then: the **span certificate's RNG-marginal gate** (M9, see below), the β>0 kernel's 18% of unused work (M9), parallel tempering, slice kernel, mode-feature discovery. |
| **Blockers** | none |
