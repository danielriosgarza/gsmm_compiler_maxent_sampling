...
| Field | Value |
|---|---|
| **Active milestone** | **M10 — extensions.** v1 (M0–M9) complete; **M10.2d (L0 caching) gate passed 2026-07-17** |
| **Status** | ⚠️ **938 tests green under *ambient* threads; 937 + 1 failure under `OMP_NUM_THREADS=1`** (`test_the_chains_mix_and_the_diagnostics_say_so`, R̂ 1.1654 vs a 1.15 bar) — **the suite and the CLI cannot currently both be green, and the thread count chooses** (see Blockers + Verify). ruff + mypy --strict clean · **the DAG's two expensive nodes are content-addressed**: `prepare_model` 22.9 s → **0.645 s** warm (35×), `T₁` key and `s_J` bit-identical cold vs warm vs no-cache; a warm run **never imports cobra**. 🔴 **But `maxent sample` FAILS from a clean cache under default threading** — see Blockers. |
| **Next action** | 🔴 **M10.2e — pin the BLAS threads, because the L3 artifact is not a function of its key.** Measured, reproducibly: the ambient BLAS thread count changes the **basis**, and the L3 key does not capture it — one key, two geometries. `OMP_NUM_THREADS=1` → basis `d35fe4fccf`, `T₀` `539da9cfc640`; unset or `4` → basis `970f8dddac`, `T₀` `d0f3bee2ce8f`. **Both bases are valid** — identical M4 span certificates (`max_width` 1.80e-12, 0 inconclusive), same dimension 46, same `cond(C_q)` 1.537e4, and both `T₀` certify — so this is a *reproducibility* defect, not a wrong-answer one. The support points are identical because HiGHS is pinned to `threads=1`; the basis is NumPy/BLAS, which reduces in a thread-dependent order. ⚠️ **This is a gap in the plan, not drift — and the tracker's first draft of this entry got it wrong** (see the section below). §1.2's thread rule is a sub-bullet of *"Sampling: process pool"* whose own parenthetical says its purpose is worker **oversubscription**; it is implemented and works (`run_batch` pins before spawning, so each worker's fresh NumPy inherits it). **Nothing ever asked for parent-side geometry determinism.** Codex's design (M10.2e review, AGREE): keep the two policies **separate** — the worker thread limit is performance, applied pre-spawn; an **L3 BLAS limit** is a *forced reproducibility* policy, **scoped around L3 construction** and **cache-versioned**. Any fix **must bump the L3 key**, or caches already holding the ambient-thread basis stay valid hits. Add **visibility beside elimination**: record the L3 recipe key, the L3 **content** key, the basis and `T₀` hashes, the determinism-policy version, the effective thread limit, and the BLAS vendor/version/architecture — that alone would have made *"one recipe key produced two content keys"* visible immediately, while the certificate went on failing closed. The basis hash belongs in **content identity and manifests, never in the pre-build lookup key** (circular). And §1.1's promise needs restating: *within a declared numerical-runtime profile* a recipe key rebuilds deterministically; **across profiles byte equality is not promised** — unrestricted cross-machine cache sharing and strict byte identity cannot both be had with ordinary floating-point libraries. ⚠️ **Recorded, not chased (agreed):** one of the two valid bases yields a `T₁` whose `certify_reachable_mass_balance` LP returns **kUnknown** — a solver failure on a demonstrably valid input. Pinning threads makes that basis unreachable *by default* and does **not** fix the fragility; any model or seed could produce the unlucky `T₁`. That is M9's own finding resurfacing in M9's own replacement gate: *a bar a valid input clears only 2 times in 3 is not a tolerance, it is a coin flip.* Reproduction: basis `970f8dddac` (build with `OMP_NUM_THREADS` unset), then `certify_reachable_mass_balance(T₁, reduced)`. **Then M10.2c — overlap `prepare_model` with sampling across models; §1.2 already mandates *that* too** ("process models so their per-model geometry can overlap the **sampling** of earlier models"): the pool is global ✓, but `run_batch` is `for spec in specs: _run_one_model(...)` and blocks on every future, so no overlap exists. The tracker's own recorded "two-phase pool dispatch" (parallelise a pilot's 4 chains) is the **weaker** remedy — ≤4×, 4 of 14 workers busy. Then: a **metadata digest in `cache.ArtifactCache`** (it hashes every array and *trusts the meta* — deferred by agreement, an all-layer property); β→performance calibration (spec §22.3 — now cheap); the **span certificate's RNG-marginal gate** (M9, below); the β>0 kernel's 18% of unused work (M9); parallel tempering; slice kernel; mode-feature discovery. |
| **Blockers** | 🔴 **`gsmm-compiler maxent sample` fails from a clean cache unless BLAS threads are pinned** (a **v1** defect, present at M10.2b's HEAD, found by M10.2d's CLI check and *not caused by it*). It **fails closed** — M9's certificate refuses to sample rather than sampling something wrong — so no incorrect numbers were ever produced. Reproduce: `env OMP_NUM_THREADS= .venv/bin/gsmm-compiler maxent sample $M --cache-dir <fresh>` → `LPNotOptimalError: LP 'reachable_mass_balance' … kUnknown`; the same command with `OMP_NUM_THREADS=1` completes. **This file's own "Verify current state" command therefore fails from a clean state**; every recorded CLI verification, mine included, reused a cache warmed by a threads-pinned probe. Fix is M10.2e (next action). |
| **Last updated** | 2026-07-16 |

### 🔴 Found by M10.2d, being fixed as M10.2e: **2.7e-15 of BLAS noise decides whether the package runs**

`gsmm-compiler maxent sample` **fails from a clean cache under default threading**, and has since at
least M10.2a. It **fails closed** — M9's certificate refuses to sample rather than sampling something
wrong — so **no incorrect numbers were ever produced.** The measured chain, end to end:
...
**So pinning the threads (M10.2e) fixes finding 1 and hides finding 4; it does not fix it.** Another
seed, model or machine can still land on a `T₁` the certificate cannot solve. Chasing that was
**deliberately deferred** — reproduction above, and it needs `certify`'s LP formulation looked at,
...
warmed by a **threads-pinned probe**. This file's own "Verify current state" command fails from a
clean state.
...
🔴 **The suite and the CLI cannot currently both be green, and the thread count is what chooses.**
Measured, deterministic (not flaky — repeated runs agree):

| | `pytest` (938) | `maxent sample`, clean cache |
|---|---|---|
| **ambient threads** | ✅ 938 passed | 🔴 fails (`kUnknown`) |
| **`OMP_NUM_THREADS=1`** | 🔴 **937 passed, 1 failed** | ✅ completes |

The one failure is `test_m5_sampler.py::TestConvergence::test_the_chains_mix_and_the_diagnostics_say_so`:
`max_r_hat` = **1.1654** against a **1.15** bar (4 × 1500 draws, d = 46). It is the *same* defect
one layer up — the 2.7e-15 basis difference is amplified by the chain, and R̂ lands either side of an
arbitrary bar depending on the ambient environment. **M9's lesson, a third time in one session:** *a
bar a valid input clears only 2 times in 3 is not a tolerance, it is a coin flip.* **M10.2e must
settle this too**, because pinning the threads — which is M10.2e's whole point — *makes this test
fail*. The bar, the schedule, or the estimator has to be justified rather than inherited.

```bash
cd /home/mcpu/GitHub/gsmm_compiler_maxent_sampling
.venv/bin/python -V                                    # expect 3.11.15
# Recorded under AMBIENT threads (the environment the numbers below were measured in).
...
#### ⬜ M10.2b — cache the pilots + two-phase dispatch  *(NEXT — pure performance)*
Measured: the pilots are **19.2 s of serial parent work** per model (`prepare_model` 2.39 s → 21.6 s), so a restart re-runs them before resuming one chain and M9's Amdahl ceiling of 24.9× falls to ~3.65×. They are 4 independent β=0 chains — the unit M8's pool already dispatches. `NeutralPilot.content_key` is complete and unwired to any store. Payload: geometry pilot → coordinates (~2.9 MB); scale pilot → **reduced fluxes** (~16.6 MB, preserving multi-objective reuse); never reconstruct fluxes from coordinates (M9: gemv vs gemm, 1.1e-13). Codex confirmed this cut is independent of every M10.2a item.

#### ⬜ M10.3+ — the rest
β→performance calibration (spec §22.3 — now cheap: `q(β)` and `r_eff(κ)` already exist); the **span certificate's RNG-marginal gate** (below); the β>0 kernel's 18% of unused work; parallel tempering; slice-based line kernel; downstream mode-feature discovery. Each behind its own tests; none alters the validated v1 target.
