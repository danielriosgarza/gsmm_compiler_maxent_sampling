...
| Field | Value |
|---|---|
| **Active milestone** | 🔴 **M11 — v1 on the real batch.** **M11.0 (solver identity) + M11.1 (`blocked_reactions` three-state + cold escalation) gates passed 2026-07-17.** build-geometry OK **4 → 20 of 40**. *The line that read "v1 (M0–M9) complete" was measured on one model and was wrong — see below.* |
...
| **Next action** | 🔴 **M11.2 — the span gate + a *shared* cold escalation for the span sweep and support-LP discovery (BUILD_PLAN §1.6.11).** M11.1 unmasked it: **8 `kUnknown@flux_only` + 4 span** refusals are the same warm-start degradation in the stages that reuse the persistent program, which M11.1 did not touch. Two parts, per the closing `/collab`: **(a)** the span gate becomes `resolution ≤ span_tol` (measured: 12/12 seeds pass, incl. both current span failures, 37× margin; `SpanCertificate.resolution` is read by no gate today, only asserted in a test); **(b)** a shared escalation mechanism (fresh-program construction, one-shot, diagnostics) those stages call — escalating *before* an unknown commits to accumulated span/support state, else a full cold restart. ⚠️ **Measure first, do not assume**: run the paired persistent-warm-vs-per-solve-fresh experiment on the 8+4 to confirm they share the cause before building (Codex). Requires `/collab` (M4). Then **M11.3** reachability's caller-specific dual-witness path (measured CERTIFIED, 27× inside contract); **M11.4** the *end-to-end* census (rounding/pilots/sampler have **never run on an aerobe**); **M11.5** release (LICENSE, README/docs, a manifest that reads the real `strains.tsv`); and the deferred **2 *Hafnia*** certificate-floor strains. |
...
#### ⬜ M11.2 — span gate `resolution ≤ span_tol` **+ shared cold escalation for the span sweep & support discovery**
The 8 `kUnknown@flux_only` + 4 span refusals are the **same warm-start degradation** in the support-LP discovery and span sweep, which reuse the same persistent program (Codex agreed the scope is *both* the gate change *and* the escalation, as a **shared** mechanism — fresh-program construction, one-shot escalation, diagnostics — with stage-specific acceptance rules kept out of it). ⚠️ **Escalate *before* an unknown result commits to accumulated support/span state**, else the stage needs a full cold restart, not a one-solve retry (Codex). **First measurement (not an assumption)**: the same paired persistent-warm-vs-per-solve-fresh experiment that settled `blocked_reactions`, to confirm the 8+4 share the cause before building.
