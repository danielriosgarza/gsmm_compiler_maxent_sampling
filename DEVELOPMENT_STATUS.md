...
| Field | Value |
|---|---|
| **Active milestone** | **M10 — extensions.** v1 (M0–M9) complete; **M10.1 (pilot DAG) gate passed 2026-07-16** |
| **Status** | ✅ 884 tests green (776 fast + 108 slow) · ruff + mypy --strict clean · `energy_scale="pilot_sd"` closes **75.8%** of the J\*-gap at β=16 where `warmup_range` closed 13% |
| **Next action** | **M10.2 — wire the pilot DAG into `batch`/CLI.** It is reachable via the library (`calibration.calibrate`) but not yet from `gsmm-compiler maxent sample`, because it needs a **decision BUILD_PLAN §1.1 does not settle**: the cache returns a `RoundedTransform` with no `ReducedGeometry`, so re-rounding on a cache hit needs the pilot and `T₁` to enter the 4-layer DAG as a new layer (with the pilot in the key). Do **not** rush it — that is exactly the shape of the defects M4/M9 recorded. Then: β→performance calibration (spec §22.3 — now cheap, `q(β)` and `r_eff(κ)` already exist); the **span certificate's RNG-marginal gate** (M9, below); the β>0 kernel's 18% of unused work (M9); parallel tempering; slice kernel; mode-feature discovery. |
| **Blockers** | none |
...
.venv/bin/python -m pytest -q | tail -3                # expect 847 passed
...
#### ⬜ M10.2 — wire the DAG into `batch`/CLI  *(NEXT — needs a §1.1 decision first)*
Reachable today via `calibration.calibrate`, but not from `gsmm-compiler maxent sample`. `batch._load_or_build_geometry` returns a cached `RoundedTransform` **with no `ReducedGeometry`**, which `reround_transform` needs — so this requires deciding whether the pilot and `T₁` join §1.1's 4-layer cache DAG as a new layer, with the pilot's key in `T₁`'s. A real fork BUILD_PLAN does not settle; deliberately not rushed at the end of M10.1.
