...
| Field | Value |
|---|---|
| **Active milestone** | **M8 — Cache, restart, batch orchestration & production** |
| **Status** | ⬜ NOT STARTED (M0–M7 gates passed; 733 tests green) |
| **Next action** | `output.py` — run-dir layout, atomic temp+rename+fsync, per-chain + run `COMPLETE` markers, configurable storage (`full_flux` f64/f32 · `reduced`). Then the 4-layer content-addressed cache + writer-claim locking, and the batch runner over a models manifest. **Fix the M8-scoped L0 key defect first** (see below): `build_canonical_model` can stamp a model with another file's cache identity. |
| **Blockers** | none |
| **Last updated** | 2026-07-16 |

> **M8 is not math-critical** — no `/collab` gate is required. Use `/collab` only for a genuine design
> fork or a non-obvious failing test (debug mode).
...
### 📌 Open for M8 (found by the M6 collab, still unfixed — do it first)
...
together and is safe (the only production caller today), but M8 builds the cache, so this is where the
L0 key must be made honest.
...
.venv/bin/python -m pytest -q | tail -3                # expect 733 passed
