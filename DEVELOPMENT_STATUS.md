...
| Field | Value |
|---|---|
| **Active milestone** | **M10 — extensions.** v1 (M0–M9) complete; **M10.2a (DAG identity + CLI wiring) gate passed 2026-07-16** |
| **Status** | ✅ 907 tests green (799 fast + 108 slow) · ruff + mypy --strict clean · **`gsmm-compiler maxent sample --set sampler.energy_scale=pilot_sd --set sampler.pilot_reround=true` now runs the pilot DAG end-to-end**, with `T₁` certified (3.86e-11, 26× inside contract) before the scale pilot steps in its frame |
| **Next action** | **M10.2b — cache the pilots + two-phase pool dispatch.** Pure performance, and measured: the pilots are **19.2 s of *serial parent* work** per model (`prepare_model` 2.39 s → 21.6 s), so an uncached pilot means a restart re-runs 19.2 s before resuming one chain, and M9's Amdahl ceiling of 24.9× falls to ~3.65×. The pilots are 4 independent β=0 chains — exactly the unit M8's pool already dispatches. Payload (agreed with Codex): geometry pilot → coordinates only (~2.9 MB); scale pilot → **reduced fluxes** only (~16.6 MB, preserving multi-objective reuse); **never** reconstruct stored fluxes from coordinates (M9: gemv vs gemm, 1.1e-13). Then: a **metadata digest in `cache.ArtifactCache`** (it hashes every array and *trusts the meta* — deferred by agreement, it is a all-layer property not an L3 one); β→performance calibration (spec §22.3 — now cheap); the **span certificate's RNG-marginal gate** (M9, below); the β>0 kernel's 18% of unused work (M9); parallel tempering; slice kernel; mode-feature discovery. |
| **Blockers** | none |
...
`warmup_range` closed **13%**. Re-rounding (spec §17.4) lands beside it: cond(C_q) **1.54e4 → 5.36e3**
(2.87×), step_scale_ratio 0.0081 → 0.0137. Full reasoning: **BUILD_PLAN §1.6.6**.
