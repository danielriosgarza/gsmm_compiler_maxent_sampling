...
| **Status** | ✅ **971 tests green.** ruff + mypy --strict clean. **build-geometry now completes on 30 of 40 real curated strains** (was 4 when M11 opened): M11.0 named the solver in the L3 cache key; M11.1 gave `blocked_reactions` a three-state classifier + fully-cold escalation (4→20); M11.2 changed the span gate to `resolution ≤ span_tol` and added a build-wide solve session that eliminated every `kUnknown@flux_only` (20→30). Root cause of the whole M11 family: a persistent warm-started HiGHS instance degrades over hundreds of sequential re-solves — its primal path is history-sensitive while its duals stay sound-but-loose. Full reasoning: **BUILD_PLAN §1.6.11**. ⚠️ **On branch `m11-real-batch` (3 commits ahead of `main` at `c5124f8`), not yet merged.** ⚠️ Geometry is the first stage only — rounding, the pilot DAG and the sampler have **never run on an aerobe**. |
...
| **Blockers** | 🔴 **build-geometry now completes on 30 of 40 strains (was 4); the other 10 fail closed** (§1.6.11) — **6 `kUnknown@reachable_mass_balance`** (M11.3, next), 2 *Hafnia* blocked floor (deferred), 1 pumilus √k span floor (deferred), 1 Liquorilactobacillus basis-dependent span (deferred, needs a tighter certificate). `kUnknown@flux_only` is **0** (was 8), the span noise-swamped refusals **0** (were the tracker's #1). All fail **closed** — no wrong numbers ever produced. **And geometry is only the first stage** — rounding, the pilot DAG and the sampler have never run on an aerobe. Progression: 4 (pre-M11) → 20 (M11.1) → 30 (M11.2). |
| **Carried, not chased** | ✅ **The span certificate's 12.5% refusal (§1.6.10) is FIXED by M11.2** — `exhaustive` gates on `resolution ≤ span_tol`, not `n_inconclusive`; `strain_1`/`strain_11` now build. 🔴 **`certify_reachable_mass_balance` is now the *active* M11.3 item (Next action above), not carried** — the audit measured its true cause: it reads **only** `row_duals` but routes through the backend gate that raises before `getSolution()`, so it refuses on the one output it never reads. Accepting the discarded `kUnknown` duals gives a bound agreeing to 8 digits with an optimal solve; the full replay is **CERTIFIED, 27× inside the 1e-9 contract**. (This supersedes the older "fragile, thread-pinned, better-conditioned-T₁-fails" framing — that was the *symptom*; the LP-formulation cause is now measured.) **The M11 root cause unifies all of this**: warm-started HiGHS instances degrade over many re-solves (primal path history-sensitive, duals sound-but-loose), and gates that consult a primal-quality signal to veto a dual-based claim refuse valid inputs. |
...
| | `pytest` (971) | `maxent sample`, clean cache |
|---|---|---|
| **ambient threads** | ✅ 971 passed | ✅ completes |
| **`OMP_NUM_THREADS=1`** | ✅ 971 passed | ✅ completes |
...
.venv/bin/python -m pytest -q | tail -3                # expect 971 passed
...
# M11 (the real batch): build-geometry across all 40 curated strains. Expect 30 OK, 10 refuse
# (6 kUnknown@reachable_mass_balance → M11.3, 2 Hafnia blocked floor, 2 span floors deferred).
